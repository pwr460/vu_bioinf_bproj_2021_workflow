...
def get_fasta_files(fasta_folder):
    return [f for f in os.listdir(fasta_folder) if f.endswith(".fasta")]

def get_pssm(file, fasta_content, preparation_config):
    ...
    cmd = os.path.join(preparation_config["psiblast_path"], filename+'.pssm') + " -num_threads " + str(preparation_config["num_threads"]) + " -query " + \
...
def get_hhm(file, fasta_content, preparation_config):
...
def make_input(file, fasta_content, preparation_config):
    """
    20pssm + 30hhm + 7pc + 19psp
    """
    filename = file.split('.')[0]

    seq_len = len(fasta_content)

    print(str(os.listdir(preparation_config["tmp_files_path"])))
    pssm_path = os.path.join(preparation_config["pssm_path"], filename+'.pssm')
    hhm_path = os.path.join(preparation_config["hhm_path"], filename+'.hhm')
    input_path = os.path.join(preparation_config["tmp_files_path"], filename+'.inputs')

    pssm = read_pssm(pssm_path, fasta_content)
    hhm = read_hhm(hhm_path, fasta_content)

    pc7 = np.zeros((seq_len, 7))
    for i in range(seq_len):
        pc7[i] = resname_to_pc7_dict[fasta_content[i]]

    psp = np.zeros((seq_len, 19))
    for i in range(seq_len):
        psp19 = resname_to_psp_dict[fasta_content[i]]
        for j in psp19:
            psp[i][j-1] = 1
